    ...
    <section className="rounded-[1.1rem] border border-border bg-surface p-4 shadow-2xl shadow-zinc-950/20 backdrop-blur-2xl sm:p-5">
      ...
      <div className="flex justify-center">
        ...
        >
          <InstructionRenderer
            html={html}
            className="rounded-2xl border border-border bg-surface p-5 text-sm leading-7 text-foreground"
          />
        </div>
      </div>
      ...
      {fullscreen && typeof document !== "undefined" &&
        createPortal(
          <div
            className="fixed inset-0 z-[9999] flex items-center justify-center bg-background/85 p-4 backdrop-blur-md sm:p-8"
            onClick={() => setFullscreen(false)}
            role="dialog"
            aria-modal="true"
          >
            <div
              className="relative flex h-full w-full max-w-6xl flex-col rounded-2xl border border-border bg-surface shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between gap-3 border-b border-border px-5 py-4">
                <h3 className="text-lg font-semibold text-foreground">Xem trước hướng dẫn — Fullscreen</h3>
              ...
              <div className="flex-1 overflow-auto p-5 sm:p-8">
                <div className="flex justify-center">
                  ...
                  >
                    <InstructionRenderer
                      html={html}
                      className="rounded-2xl border border-border bg-surface p-6 text-base leading-8 text-foreground"
                    />
                  </div>
                </div>
              </div>
