...
export function WizardFooter({ backHref, nextHref, nextLabel = "Tiếp", onNext }: WizardFooterProps) {
  return (
    <div className="mt-5 flex flex-col gap-3 rounded-[1.1rem] border border-white/10 bg-zinc-900/58 p-4 shadow-2xl shadow-zinc-950/20 backdrop-blur-2xl sm:flex-row sm:items-center sm:justify-between sm:p-5">
      <div className="text-sm text-zinc-400">Tiếp tục sang bước kế tiếp khi đã kiểm tra nội dung hiện tại.</div>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        {backHref ? (
          <Link className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.07] px-4 text-sm font-semibold text-zinc-100 transition hover:bg-white/[0.11]" to={backHref}>
