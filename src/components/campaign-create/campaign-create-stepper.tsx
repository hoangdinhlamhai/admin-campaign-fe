...
export function CampaignCreateStepper({ activeStep = 1 }: CampaignCreateStepperProps) {
  ...
  return (
    <section className="glass-card mb-5 grid gap-3 p-4 lg:grid-cols-4">
      {campaignCreateSteps.map((step, index) => {
        const active = step.id === activeStep;
        const href = `${base}${stepSuffix[step.id]}`;
        const content = (
          <>
            <div className={`grid size-10 shrink-0 place-items-center rounded-full text-sm font-bold ${active ? "bg-brand text-brand-foreground" : "bg-indigo-400/16 text-indigo-100"}`}>
              {step.id}
            </div>
        ...
        return (
          <div className="flex items-center gap-3" key={step.id}>
            <Link className="flex min-w-0 flex-1 items-center gap-3 rounded-xl p-1 transition hover:bg-surface-2" to={href}>
              {content}
            </Link>
